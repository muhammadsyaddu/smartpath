...
<style>
    #map-container { height: min(78vh, 900px); min-height: 520px; width: 100%; }
    @media (max-width: 768px) { #map-container { height: calc(100vh - 64px - 60px); min-height: 420px; } }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadLaporan() {
        ...
        fetch('{{ route("peta.data") }}?' + params.toString(), { headers: { 'Accept': 'application/json' } })
            .then(res => {
                if (!res.ok) throw new Error('Gagal memuat data laporan');
                return res.json();
            })
            .then(data => {
                cachedLaporanData = Array.isArray(data) ? data : [];
                renderLaporan(cachedLaporanData);
                populateKategoriFilters(cachedLaporanData);
            })
            .catch(err => {
    ...
    function renderLaporan(data) {
        ...
        (data || []).forEach(item => {
            if (!item || typeof item.latitude === 'undefined' || typeof item.longitude === 'undefined') return;

            const marker = L.marker([Number(item.latitude), Number(item.longitude)], {
                icon: getMarkerIcon((item.tingkat_prioritas || '').toLowerCase(), item.status)
            });
            const popupContent = `
                <div style="min-width:200px;max-width:280px;">
                    ${item.foto_utama ? `<img src="${item.foto_utama}" alt="Foto laporan" style="width:100%;height:120px;object-fit:cover;border-radius:8px;margin-bottom:8px;">` : ''}
                    <h3 style="font-weight:600;font-size:14px;margin-bottom:4px;color:#0f172a;">${item.judul || 'Laporan Hambatan'}</h3>
                    <p style="font-size:12px;color:#64748b;margin-bottom:6px;">${item.kategori || 'Tanpa kategori'}</p>
                    <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;flex-wrap:wrap;">
                        <span style="display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:6px;font-size:11px;font-weight:600;background:#e2e8f0;color:#475569;">${item.status_label || item.status || 'Status'}</span>
                        ${item.skor_prioritas ? `<span style="font-size:11px;color:#94a3b8;">Skor: ${Number(item.skor_prioritas).toFixed(0)}</span>` : ''}
                    </div>
    ...
    function loadFasilitas() {
        fetch('{{ route("peta.fasilitas") }}', { headers: { 'Accept': 'application/json' } })
            .then(res => {
            ...
            .then(data => {
                ...
                (data || []).forEach(item => {
                    if (!item || typeof item.latitude === 'undefined' || typeof item.longitude === 'undefined') return;
                    const marker = L.marker([Number(item.latitude), Number(item.longitude)], { icon: getFacilityIcon() });
                    const popupContent = `
        ...
        data.forEach(item => {
            if (item.kategori_id && !kategoriMap.has(item.kategori_id)) {
                kategoriMap.set(item.kategori_id, {
                    id: item.kategori_id,
                    nama: item.kategori || 'Tanpa Kategori',
