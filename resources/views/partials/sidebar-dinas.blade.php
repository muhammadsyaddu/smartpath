...
>
    <div class="flex-1 overflow-y-auto">
        ...
        <nav class="px-4 py-6 space-y-1" aria-label="Menu dinas">
            {{-- Dashboard --}}
            ...
            {{-- Laporan --}}
            <div class="pt-5 pb-2">
                <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Laporan</span>
            </div>

            {{-- Verifikasi --}}
            <a href="{{ route('admin.verifikasi.index') }}"
               class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl
                      {{ request()->routeIs('admin.verifikasi.*') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <!-- Papan Klip -->
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    {{-- Profil Akun Pengguna (selalu di bawah) --}}
    <div class="p-4 border-t border-slate-200 bg-white flex-shrink-0">
        <div class="flex items-center gap-3 p-2 rounded-2xl border border-slate-200 bg-slate-50">
            ...
            <div class="min-w-0 flex-1">
                <p class="text-xs font-bold text-slate-800 truncate">{{ auth()->user()->name ?? 'Dinas PUPR' }}</p>
                <p class="text-[11px] text-slate-500 truncate">{{ auth()->user()->email ?? 'pupr@depok.go.id' }}</p>
            </div>
        </div>
    </div>
</aside>
